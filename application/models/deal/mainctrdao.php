...
class mainCtrDao extends VX_Model {
    ...
    public function getFiles($ofertaId){

        $files=$this->db->query("select id, name, fecha from of_files where id_oferta = $ofertaId;");
...
public function saveDeal($userData, $params) {
    ...
    public function saveGenerador($generadorFile, $ofertaId) {
        ...
        foreach ($sheet->getRowIterator(3) as $row) {
            ...
            if (!empty(array_filter($data_generador))) {
                $data_generador['id_oferta'] = $ofertaId;
        ...
        foreach ($sheet->getRowIterator(19) as $row) {
            ...
            if (!empty(array_filter($data_generador_dos))) {
                $data_generador_dos['id_oferta'] = $ofertaId;
    ...
    public function generarDatosEmpresa($params) {
        $ofertaId = $params["ofertaId"];
        $query = "SELECT of_ofertas.createdDate, of_clientes.nombre, of_clientes.razonSocial
                  FROM of_ofertas
                  INNER JOIN of_clientes ON of_ofertas.clienteid = of_clientes.clienteid
                  WHERE of_ofertas.ofertaid = " . $ofertaId;
    ...
    public function validarSeguimiento($params) {
        ...
        $query = $this->db->query("SELECT paso FROM of_pasos WHERE pasoId = " . $params["findPaso"]);
        $paso = $query->row()->paso;
    
        $query1 = $this->db->query("SELECT name FROM employees WHERE employees_id = " . $params["employeeid"]);
        $employee = $query1->row()->name;
    ...
    public function validarPrecio($ofertaId) {
        ...
        WHERE p.fecha = DATE_FORMAT(NOW(), '%d/%b/%Y')
    ...
    public function validarBtnPasoSi5($ofertaId){
        $query = $this->db->query("  
        SELECT
        IF(aprobado_paso5 = 1 , TRUE, FALSE) AS resultado
        FROM
        of_datos_seguimiento
        WHERE
        ofertaId = $ofertaId;
    ...
    public function getAuditoria($ofertaId){
        $auditoria=$this->db->query("SELECT * FROM of_auditoria_validar WHERE oferta_id = $ofertaId; ");
        return $auditoria->result_array();
    }
    ...
    public function crearPorcentajePerfil($ofertaId, $GB){
        ...
        $restabla = $this->db->query('select * from of_perfil where oferta_id='.$ofertaId.';');
    ...
    public function crearFacturacionCFESSB($ofertaId, $GB, $params){
        ...
        $restabla = $this->db->query('select * from of_calculo_facturacion_cfessb where of_oferta='.$ofertaId.';');
        return $restabla->result_array();
    }
